...
fn main(){
    ...
    let op=a.get(3).map(String::as_str).unwrap_or("cumsum");
    let it:usize=a.get(4).and_then(|s|s.parse().ok()).unwrap_or(6);
    let index=Index::new_known_unique_int64_unit_range(0,n);
    let mut cols=BTreeMap::new();
    cols.insert("k".to_string(),Column::from_i64_values((0..n).map(|i|(sm(i,0)%g as u64) as i64).collect()));
    ...
    for _ in 0..it{let t=Instant::now();
        match op{"cumsum"=>{std::hint::black_box(df.groupby(&["k"]).unwrap().cumsum().unwrap());}"cummax"=>{std::hint::black_box(df.groupby(&["k"]).unwrap().cummax().unwrap());}"shift"=>{std::hint::black_box(df.groupby(&["k"]).unwrap().shift(1).unwrap());}"diff"=>{std::hint::black_box(df.groupby(&["k"]).unwrap().diff(1).unwrap());}_=>panic!()};
