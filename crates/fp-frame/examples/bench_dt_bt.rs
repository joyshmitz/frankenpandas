...
fn main(){
    ...
    let s=Series::new("t",idx.clone(),Column::from_values((0..n).map(|i| Scalar::Datetime64(base + (sm(i,3)%(6*365*24*3600)) as i64 * 1_000_000_000)).collect()).unwrap()).unwrap();
    t("dt_year", || { std::hint::black_box(s.dt().year().unwrap()); });
    t("dt_dayofweek", || { std::hint::black_box(s.dt().dayofweek().unwrap()); });
    t("dt_floor_D", || { std::hint::black_box(s.dt().floor("D").unwrap()); });
    t("dt_normalize", || { std::hint::black_box(s.dt().normalize().unwrap()); });
}
