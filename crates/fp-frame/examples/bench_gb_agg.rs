//! df.groupby(k).{median,quantile,nunique,sem,first,last,std,skew} @1M. bench_gb_agg <n> <g> <op>
...
fn main() {
    ...
    cols.insert(
        "k".to_string(),
        Column::from_i64_values((0..n).map(|x| (sm(x, 0) % g as u64) as i64).collect()),
    );
    cols.insert(
        "v".to_string(),
    ...
    println!("gba_{op} n={n} g={g}: best={best}ns");
}
