...
| Series.map Float64 dense integer-key table (0jdij, bench_map_cc) | 2M f64, 50-entry zero-based full-coverage map, pinned CPU 7 | 12.116 ms | 17.924 ms | **0.68× (1.48× slower)** | ✅ KEEP FP-side gain / 🔴 still pandas loss — stale 1.21× win row did NOT reproduce on this host. Dense direct-address table for finite integer-valued Float64 keys (zero-based complete-map fast lane: `v as usize` + round-trip/range guard) cuts current FP 21.326→17.924 ms = **1.19× FP-side** and preserves exact Float-Float map semantics. Rejected intermediate `trunc()` gate as ~0-gain (21.3→21.2 ms). Conformance green: 18 `series_map_*` tests + 4 FxHash/factorize order tests; clippy green. Residual = output construction + per-row exact-key guard still behind pandas' map engine; tracked as follow-up |
