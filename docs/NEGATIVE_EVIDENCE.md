...
| Series.map Float64 + replace tolerance bug | 2M f64, 50-entry full-coverage map | 26.05 ms | 192.7 ms | **0.135× (7.4× slower)** | 🔴 LOSS + PARITY BUG — map Float64 keys use semantic_eq (1e-14 tolerant + cross-type) via try_build_mapping_index returning None for Float64 → O(n·m) linear scan. pandas map is EXACT (1.0+5e-15 → nan). Proper fix = coordinated golden-gated parity change (uniform exact Float64 keys across replace/map/DataFrame.replace) which ALSO unlocks the hash fast path (the 7.4× perf). NOT a bit-transparent quick win — deferred to coordinated fix. Bench bench_map_cc |
