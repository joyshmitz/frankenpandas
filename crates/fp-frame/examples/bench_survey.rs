...
fn main() {
    ...
    timeit("round(2)", || { std::hint::black_box(s.round(2).unwrap().len()); });
    timeit("diff(1)", || { std::hint::black_box(s.diff(1).unwrap().len()); });
    ...
    timeit("sort_values", || { std::hint::black_box(s.sort_values(true).unwrap().len()); });
}
