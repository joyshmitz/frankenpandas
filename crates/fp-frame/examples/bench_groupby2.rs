//! groupby by TWO Utf8 keys (multi-key composite) .sum(). Run: -- 1000000 100 50 15
...
fn main() {
    ...
    for _ in 0..iters {
        let t = Instant::now();
        std::hint::black_box(df.groupby(&["a", "b"]).unwrap().sum().unwrap());
        let e = t.elapsed().as_nanos();
        if e < best {
            best = e;
        }
    }
    println!("groupby2_utf8 n={n} c1={c1} c2={c2}: best={best}ns");
}
