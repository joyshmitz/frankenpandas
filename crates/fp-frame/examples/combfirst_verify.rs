...
fn main() {
    let n = 2500usize;
    let mk = |seed: u64, withnull: bool| {
        let mut lbl: Vec<i64> = (0..n)
            .map(|i| ((sm(i, seed) % 2000) as i64) - 700)
            .collect();
        lbl.sort();
        lbl.dedup();
        let idx = Index::new(
        ...
        let vals: Vec<Scalar> = (0..lbl.len())
            .map(|i| {
                if withnull && sm(i, 3).is_multiple_of(3) {
                    Scalar::Null(NullKind::Null)
                } else {
                    Scalar::Float64((sm(i, seed + 1) % 100) as f64)
                }
            })
            .collect();
    ...
    let (a, al, av) = mk(1, true);
    let (b, bl, bv) = mk(2, false);
    let r = a.combine_first(&b).unwrap();
    let idx = r.index().labels();
    let c0 = r.column("c0").unwrap().values();
    let mut f = std::fs::File::create("/tmp/fp_combfirst.txt").unwrap();
