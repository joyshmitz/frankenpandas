...
fn main(){
    ...
    let col=Column::from_values((0..n).map(|i| if sm(i,1)%5==0 {Scalar::Null(NullKind::NaN)} else {Scalar::Float64((sm(i,9)%100000) as f64)}).collect()).unwrap();
    let positions: Vec<usize> = (0..n).filter(|&i| sm(i,1)%5!=0).collect();
    println!("kept={}", positions.len());
    t("index.take", || { std::hint::black_box(idx.take(&positions)); });
    t("col.take_positions", || { std::hint::black_box(col.take_positions(&positions)); });
    let _=col;
}
