...
fn main(){
    ...
    let v2=Series::new("v2",idx.clone(),Column::from_values((0..n).map(|i| Scalar::Float64((sm(i,9)%100000) as f64)).collect()).unwrap()).unwrap();
    ...
    t("ewm_mean", || { std::hint::black_box(v.ewm(Some(20.0),None).mean().unwrap()); });
    t("ewm_std", || { std::hint::black_box(v.ewm(Some(20.0),None).std().unwrap()); });
    t("ewm_cov", || { std::hint::black_box(v.ewm(Some(20.0),None).cov(&v2).unwrap()); });
    t("expanding_sum", || { std::hint::black_box(v.expanding(None).sum().unwrap()); });
    t("expanding_std", || { std::hint::black_box(v.expanding(None).std().unwrap()); });
    t("interpolate", || { std::hint::black_box(vn.interpolate().unwrap()); });
    t("ewm_mean_nan", || { std::hint::black_box(vn.ewm(Some(20.0),None).mean().unwrap()); });
}
