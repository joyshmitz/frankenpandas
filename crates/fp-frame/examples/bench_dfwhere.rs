...
fn main() {
    ...
    for c in 0..ncols {
        let name = format!("c{c}");
        cols.insert(name.clone(), Column::from_f64_values((0..n).map(|i| (i as f64) * 0.5 + c as f64).collect()));
        conds.insert(name, Column::from_bool_values((0..n).map(|i| (i + c) % 2 == 0).collect()));
    }
    ...
    println!("df.where f64 {}x{} : best={:.2}ms", n, ncols, best as f64 / 1e6);
}
