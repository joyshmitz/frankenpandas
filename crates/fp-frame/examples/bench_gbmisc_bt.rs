...
fn main(){
    ...
    t("gb_transform_mean", || { std::hint::black_box(v.groupby(&k).unwrap().transform("mean").unwrap()); });
    t("gb_cumsum", || { std::hint::black_box(v.groupby(&k).unwrap().cumsum().unwrap()); });
    t("gb_first", || { std::hint::black_box(v.groupby(&k).unwrap().first().unwrap()); });
    t("gb_nth0", || { std::hint::black_box(v.groupby(&k).unwrap().nth(0).unwrap()); });
    t("gb_head3", || { std::hint::black_box(v.groupby(&k).unwrap().head(3).unwrap()); });
    t("mode", || { std::hint::black_box(v.mode().unwrap()); });
    t("gb_agg_firstlast", || { std::hint::black_box(v.groupby(&k).unwrap().agg(&["first","last","sum"]).unwrap()); });
}
