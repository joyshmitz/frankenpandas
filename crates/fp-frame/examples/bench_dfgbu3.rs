...
fn main() {
    ...
    let df = DataFrame::new_with_column_order(
        Index::new(labels),
        cols,
        vec!["k".into(), "a".into(), "bo".into()],
    )
    .unwrap();
    timeit("first", || {
        std::hint::black_box(df.groupby(&["k"]).unwrap().first().unwrap().shape());
    });
    timeit("last", || {
        std::hint::black_box(df.groupby(&["k"]).unwrap().last().unwrap().shape());
    });
    timeit("all", || {
        std::hint::black_box(df.groupby(&["k"]).unwrap().all().unwrap().shape());
    });
    timeit("any", || {
        std::hint::black_box(df.groupby(&["k"]).unwrap().any().unwrap().shape());
    });
    timeit("sem", || {
        std::hint::black_box(df.groupby(&["k"]).unwrap().sem().unwrap().shape());
    });
    timeit("head", || {
        std::hint::black_box(df.groupby(&["k"]).unwrap().head(5).unwrap().shape());
    });
    timeit("tail", || {
        std::hint::black_box(df.groupby(&["k"]).unwrap().tail(5).unwrap().shape());
    });
    timeit("nth", || {
        std::hint::black_box(df.groupby(&["k"]).unwrap().nth(0).unwrap().shape());
    });
}
