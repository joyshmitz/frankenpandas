...
fn build(n:usize,m:usize)->DataFrame{
    ...
    for c in 0..m {
        let vv:Vec<Scalar>=(0..n).map(|i| if sm(i,(c as u64)*97+1)%10==0 {Scalar::Null(NullKind::NaN)} else {Scalar::Float64((sm(i,(c as u64)*13+7)%100000) as f64)}).collect();
...
fn main(){
    for (n,m) in [(500_000usize,8usize),(500_000,12),(200_000,10),(200_000,15)]{
        let df=build(n,m);
        timeit(&format!("corr n={n} m={m}"), || { std::hint::black_box(df.corr().unwrap()); });
    }
}
