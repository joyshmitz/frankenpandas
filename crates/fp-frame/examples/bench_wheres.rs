...
fn mkf(off:i64,n:usize,seed:u64,nullable:bool)->Series{
    ...
    let v:Vec<Scalar>=(0..n).map(|i| if nullable && sm(i,seed)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Float64((sm(i,seed)%100) as f64)}).collect();
...
fn main(){
    ...
    let s=mkf(0,n,1,true); let other=mkf(200_000,n,2,true);
    timeit("s.where UNALIGNED nullable (1M)", || { std::hint::black_box(s.where_cond_series(&cond,&other).unwrap()); });
    let s2=mkf(0,n,1,false); let other2=mkf(200_000,n,2,false);
    timeit("s.where UNALIGNED all-valid (1M)", || { std::hint::black_box(s2.where_cond_series(&cond,&other2).unwrap()); });
}
