...
fn dig(df:&DataFrame)->u64{ let mut h=1469598103934665603u64; for v in df.column("value").unwrap().values().iter(){ let b=format!("{:?}",v); for by in b.bytes(){h^=by as u64;h=h.wrapping_mul(1099511628211);} } h }
fn main(){
    let n=200_000usize; let idx=Index::from_range(0,n as i64,1);
    ...
    println!("stack_digest={} dtype={:?}", dig(&df.stack().unwrap()), df.stack().unwrap().column("value").unwrap().dtype());
    t("stack_mixed", || { std::hint::black_box(df.stack().unwrap()); });
}
