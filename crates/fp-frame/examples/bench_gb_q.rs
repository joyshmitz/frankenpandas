...
fn main() {
    ...
    cols.insert(
        "k".to_string(),
        Column::from_i64_values((0..n).map(|i| (i as i64) % g).collect()),
    );
    cols.insert(
        "a".to_string(),
    ...
    for _ in 0..it {
        ...
        match op {
            "quantile" => {
                std::hint::black_box(df.groupby(&["k"]).unwrap().quantile(0.25).unwrap());
            }
            "describe" => {
                std::hint::black_box(df.groupby(&["k"]).unwrap().describe().unwrap());
            }
            _ => panic!(),
        };
