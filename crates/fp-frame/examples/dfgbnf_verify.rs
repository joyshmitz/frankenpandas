...
fn dumpdf(f:&mut std::fs::File, tag:&str, df:&DataFrame){
    ...
    for cn in ["a".to_string(),"b".to_string()] {
        ...
        for i in 0..idx.len(){
            let lbl=match &lbls[i]{ fp_index::IndexLabel::Utf8(v)=>v.clone(), o=>format!("{o:?}")};
            let v=match &col[i]{ Scalar::Null(_)=>"NA".into(), Scalar::Float64(x)=>format!("{x:.5}"), Scalar::Int64(x)=>format!("{x}"), o=>format!("{o:?}")};
            rows.push((lbl,v));
        }
        rows.sort();
...
fn main(){
    let n=6000usize; let card=30usize;
    let keys:Vec<Scalar>=(0..n).map(|i| Scalar::Utf8(format!("cat{}", sm(i,1)%card as u64))).collect();
    let a:Vec<Scalar>=(0..n).map(|i| { let g=sm(i,1)%card as u64; if g==4 || sm(i,7)%4==0 {Scalar::Null(NullKind::Null)} else {Scalar::Float64((sm(i,9)%100) as f64 - 50.0)} }).collect();
    ...
    let ks:Vec<String>=keys.iter().map(|x| if let Scalar::Utf8(v)=x {v.clone()} else {"NA".into()}).collect();
    let av:Vec<String>=a.iter().map(|x| match x {Scalar::Float64(v)=>format!("{v}"),_=>"NA".into()}).collect();
    ...
    dumpdf(&mut f,"sum",&df.groupby(&["k".into()]).unwrap().sum().unwrap());
    dumpdf(&mut f,"mean",&df.groupby(&["k".into()]).unwrap().mean().unwrap());
    dumpdf(&mut f,"max",&df.groupby(&["k".into()]).unwrap().max().unwrap());
    dumpdf(&mut f,"min",&df.groupby(&["k".into()]).unwrap().min().unwrap());
    dumpdf(&mut f,"count",&df.groupby(&["k".into()]).unwrap().count().unwrap());
    dumpdf(&mut f,"std",&df.groupby(&["k".into()]).unwrap().std().unwrap());
