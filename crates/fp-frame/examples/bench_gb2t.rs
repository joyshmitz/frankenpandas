...
fn main() {
    ...
    let op = a.get(1).map(String::as_str).unwrap_or("ffill");
    ...
    let df =
        DataFrame::new_with_column_order(index, cols, vec!["k1".into(), "k2".into(), "a".into()])
            .unwrap();
    ...
    for _ in 0..6 {
        ...
        match op {
            "ffill" => {
                std::hint::black_box(df.groupby(&["k1", "k2"]).unwrap().ffill(None).unwrap());
            }
            "ngroup" => {
                std::hint::black_box(df.groupby(&["k1", "k2"]).unwrap().ngroup().unwrap());
            }
            "transform_mean" => {
                std::hint::black_box(
                    df.groupby(&["k1", "k2"])
