...
fn dig(s:&Series)->u64{ // digest of (label,count) output in order
    let mut h=1469598103934665603u64;
    for l in s.index().labels().iter(){ let b=format!("{:?}",l); for by in b.bytes(){ h^=by as u64; h=h.wrapping_mul(1099511628211);} }
    for v in s.column().values().iter(){ let b=format!("{:?}",v); for by in b.bytes(){ h^=by as u64; h=h.wrapping_mul(1099511628211);} }
    h
}
fn main(){
    let n=2_000_000usize; let idx=Index::from_range(0,n as i64,1);
    for (card,lbl) in [(100u64,"lowcard-100"),(50000,"mid-50k"),(80000,"card-80k"),(300000,"highmid-300k"),(2_000_000,"extreme")] {
        let v=Series::new("v",idx.clone(),Column::from_values((0..n).map(|i| Scalar::Float64((sm(i,7)%card) as f64)).collect()).unwrap()).unwrap();
        println!("digest {lbl}={}", dig(&v.value_counts().unwrap()));
        t(&format!("vc {lbl}"), || { std::hint::black_box(v.value_counts().unwrap()); });
    }
}
