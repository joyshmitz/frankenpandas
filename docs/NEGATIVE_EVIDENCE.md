...
| Series.loc[[labels]] arbitrary Int64 index (bench_loc_cc) | 2M f64, step-2 sorted index, select 1000 | 0.136 ms | 725 ms | **0.0002× (5340× slower)** | 🔴 LOSS — non-unit-range path builds a pointer-key FxHashMap<&IndexLabel,Vec<usize>> over the WHOLE index PER CALL: O(n) + one Vec alloc per label (2M tiny Vecs) + labels() materialization. pandas caches/binary-searches (sorted) ⇒ ~O(k). Tried a sorted get_loc fast path (locsrt) — REVERTED ~0-gain: Index::position only binary-searches LAZY-int64-backed indexes, not materialized Vec<IndexLabel>. Real fix = fp-index (position-for-materialized-sorted, or cached index hashtable). Out of clean fp-frame lane |
