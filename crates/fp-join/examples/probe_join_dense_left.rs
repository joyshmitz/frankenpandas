...
fn build(n:usize)->(DataFrame,DataFrame){
  ...
  // unordered unique left keys; unordered right keys with duplicates, bounded span.
  let lk:Vec<i64>=(0..n as i64).map(|i|(i*2654435761i64).rem_euclid(n as i64)).collect();
  let rk:Vec<i64>=(0..n as i64).map(|i|(i*7)%(n as i64)).collect();
...
fn main(){
  ...
  { let (l,r)=build(n);
    for _ in 0..3{black_box(merge_dataframes_on(&l,&r,&["key"],JoinType::Left).unwrap().columns.len());}
    let st=Instant::now(); let mut k=0usize;
    for _ in 0..it{k^=black_box(merge_dataframes_on(&l,&r,&["key"],JoinType::Left).unwrap().columns.len());}
    println!("dense_left_amortized   : {:.3} ms/call (k={k})", st.elapsed().as_secs_f64()*1000.0/it as f64);
  }
  { let mut tot=0.0; let mut k=0usize;
    for _ in 0..3{ let (l,r)=build(n); black_box(merge_dataframes_on(&l,&r,&["key"],JoinType::Left).unwrap().columns.len());}
    for _ in 0..it{ let (l,r)=build(n); let st=Instant::now();
      k^=black_box(merge_dataframes_on(&l,&r,&["key"],JoinType::Left).unwrap().columns.len()); tot+=st.elapsed().as_secs_f64()*1000.0;}
    println!("dense_left_oneshot     : {:.3} ms/call (k={k})", tot/it as f64);
  }
}
