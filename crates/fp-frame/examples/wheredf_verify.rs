...
fn mkf(off:i64,n:usize,ncol:usize)->DataFrame{
    ...
    for c in 0..ncol {
        let nm=format!("c{c}");
        let v:Vec<Scalar>=(0..n).map(|i| if sm(i,c as u64+7)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Float64((sm(i,c as u64+1)%100) as f64)}).collect();
...
fn dump(tag:&str, df:&DataFrame){
    ...
    for c in 0..3 {
        ...
        for i in 0..idx.len() {
            let key=match &idx[i]{fp_index::IndexLabel::Int64(x)=>*x,_=>i64::MIN};
            let val=match &col[i]{Scalar::Float64(f)=>if f.is_nan(){"NaN".into()}else{format!("{f:.4}")},Scalar::Null(_)=>"NaN".into(),o=>format!("{o:?}")};
            print!("c{c}:{key}:{val} ");
        }
    }
    println!();
