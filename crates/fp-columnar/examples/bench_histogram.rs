...
fn golden() -> String {
    ...
    out.push_str(&format!("nonstrict:{}\n", dump(&data.histogram(&[0.0, 1.0, 1.0, 2.0, 3.0]).unwrap())));
    // nullable column
    let nf = Column::new(
        DType::Float64,
        vec![Scalar::Float64(0.5), Scalar::Null(NullKind::NaN), Scalar::Float64(2.5)],
    )
    .unwrap();
...
fn main() {
    ...
    println!("TIMING n={n} bins={n_bins} histogram={:.3}ms", d.as_secs_f64() * 1e3);
}
