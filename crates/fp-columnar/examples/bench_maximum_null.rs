//! Bench for `Column::maximum` on nullable columns after adding the typed nullable
//! pairwise arm (typed_nan_propagate_binary) — folds f(x,y) off both raw slices via
//! get_present, instead of the generic per-Scalar loop that boxes a Vec<Scalar> output
//! + Column::new.
//!
//! NEW = a.maximum(&b). CONTROL = a replica of the old generic loop (Vec<Scalar> +
//! Column::new) over the (cached) values() of both ⇒ conservative lower bound. Slots
//! asserted equal. (minimum shares the helper.)
//!
//! Run: cargo run -p fp-columnar --release --example bench_maximum_null -- 5000000 30

use fp_columnar::{Column, ValidityMask};
use fp_types::{DType, Scalar};

fn ref_max_col(a: &[Scalar], b: &[Scalar]) -> Column {
...
fn main() {
    ...
    for _ in 0..iters {
        let t = std::time::Instant::now();
        let r = ref_max_col(a.values(), b.values());
        best_c = best_c.min(t.elapsed().as_nanos());
        std::hint::black_box(&r);
    }
    let got = a.maximum(&b).unwrap();
    let want = ref_max_col(&av, &bv);
    for k in [0usize, 1, 2] {
        assert_eq!(
    ...
    println!(
        "maximum f64null_x_i64null n={n} NEW={:>7.2}ms CONTROL={:>7.2}ms speedup={:.3}x",
        best_t as f64 / 1e6,
        best_c as f64 / 1e6,
