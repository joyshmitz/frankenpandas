...
fn main(){
    ...
    t("pct_change", || { std::hint::black_box(v.pct_change(1).unwrap()); });
    t("autocorr", || { std::hint::black_box(v.autocorr(1).unwrap()); });
    t("rank_pct", || { std::hint::black_box(v.rank_with_pct("average", true, "keep", true).unwrap()); });
}
