...
fn main(){
    ...
    t("dfgb_agg_fl", || { std::hint::black_box(df.groupby(&["k"]).unwrap().agg_list(&["first","last","sum","mean"]).unwrap()); });
    let r=df.groupby(&["k"]).unwrap().agg_list(&["first","last","sum","mean"]).unwrap();
    let mut d=0u64;
    for c in ["a_first","a_last","a_sum","a_mean","b_first","b_last","b_sum","b_mean"]{ if let Some(col)=r.column(c){ for x in col.values().iter(){ if let Scalar::Float64(f)=x { d=d.wrapping_mul(1099511628211).wrapping_add(f.to_bits()); } } } }
    println!("digest={d}");
}
