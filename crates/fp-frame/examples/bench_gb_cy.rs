...
fn main() {
    ...
    cols.insert(
        "k".to_string(),
        Column::from_i64_values((0..n).map(|i| (sm(i, 0) % g as u64) as i64).collect()),
    );
    cols.insert(
        "a".to_string(),
    ...
    for _ in 0..it {
        ...
        match op {
            ...
            "cummax" => {
                std::hint::black_box(df.groupby(&["k"]).unwrap().cummax().unwrap());
            }
            "shift" => {
                std::hint::black_box(df.groupby(&["k"]).unwrap().shift(1).unwrap());
            }
            "diff" => {
                std::hint::black_box(df.groupby(&["k"]).unwrap().diff(1).unwrap());
            }
            _ => panic!(),
        };
