...
fn dumpdf(f:&mut std::fs::File, tag:&str, df:&DataFrame){
    ...
    for i in 0..idx.len(){
        ...
        let v=match &col[i]{ Scalar::Int64(x)=>format!("i{x}"), Scalar::Float64(x)=>format!("f{x:.5}"), Scalar::Null(_)=>"NA".into(), o=>format!("{o:?}")};
        rows.push((lbl,v));
    }
    rows.sort();
...
fn main(){
    ...
    for (tag,utf8,card) in [("bounded",false,30usize),("eager",true,30usize)]{
        ...
        let a:Vec<Scalar>=(0..n).map(|i| { let g=sm(i,1)%card as u64; if g==4 || sm(i,7)%4==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((sm(i,9)%50) as i64 + 1)} }).collect();
        ...
        for op in ["sum","mean","max","min","count","prod","var","std","median","first","last"]{
            let g=df.groupby(&["k".into()]).unwrap();
            let r=match op {"sum"=>g.sum(),"mean"=>g.mean(),"max"=>g.max(),"min"=>g.min(),"count"=>g.count(),"prod"=>g.prod(),"var"=>g.var(),"std"=>g.std(),"median"=>g.median(),"first"=>g.first(),_=>g.last()}.unwrap();
