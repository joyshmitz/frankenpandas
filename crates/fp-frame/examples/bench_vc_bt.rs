...
fn main(){
    ...
    let av:Vec<Scalar>=(0..n).map(|i| Scalar::Float64((sm(i,7)%100000) as f64)).collect();
    let sa=Series::new("v", idx.clone(), Column::from_values(av).unwrap()).unwrap();
    // nullable 20%, same distinct
    let nv:Vec<Scalar>=(0..n).map(|i| if sm(i,1)%5==0 {Scalar::Null(NullKind::NaN)} else {Scalar::Float64((sm(i,7)%100000) as f64)}).collect();
    let sn=Series::new("v", idx.clone(), Column::from_values(nv).unwrap()).unwrap();
    // low-card variants (~500 distinct)
    let al:Vec<Scalar>=(0..n).map(|i| Scalar::Float64((sm(i,7)%500) as f64)).collect();
    let sal=Series::new("v", idx.clone(), Column::from_values(al).unwrap()).unwrap();
    let nl:Vec<Scalar>=(0..n).map(|i| if sm(i,1)%5==0 {Scalar::Null(NullKind::NaN)} else {Scalar::Float64((sm(i,7)%500) as f64)}).collect();
    let snl=Series::new("v", idx.clone(), Column::from_values(nl).unwrap()).unwrap();
    t("vc_allvalid_highcard", || { std::hint::black_box(sa.value_counts().unwrap()); });
    t("vc_nullable_highcard", || { std::hint::black_box(sn.value_counts().unwrap()); });
    t("vc_allvalid_lowcard", || { std::hint::black_box(sal.value_counts().unwrap()); });
    t("vc_nullable_lowcard", || { std::hint::black_box(snl.value_counts().unwrap()); });
}
