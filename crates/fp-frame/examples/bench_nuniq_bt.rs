...
fn main(){
    ...
    let uniq=Series::new("u",idx.clone(),Column::from_values((0..n).map(|i| Scalar::Float64(i as f64 + 0.5)).collect()).unwrap()).unwrap();
    println!("hc_distinct={} uniq_distinct={}", hc.nunique(), uniq.nunique());
    t("nunique_highcard", || { std::hint::black_box(hc.nunique()); });
    t("nunique_allunique", || { std::hint::black_box(uniq.nunique()); });
}
