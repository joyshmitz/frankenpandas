...
fn main() {
    ...
    .unwrap();
    t("where", || {
        std::hint::black_box(v.where_cond(&cond, Some(&Scalar::Float64(0.0))).unwrap());
    });
    t("mask", || {
        std::hint::black_box(v.mask(&cond, Some(&Scalar::Float64(0.0))).unwrap());
    });
    t("ffill", || {
        std::hint::black_box(v2.ffill(None).unwrap());
    });
    t("bfill", || {
        std::hint::black_box(v2.bfill(None).unwrap());
    });
    t("dropna", || {
        std::hint::black_box(v2.dropna().unwrap());
    });
}
