...
fn main() {
    ...
    timeit("isin_i64(100)", || { std::hint::black_box(si.isin(&needles).unwrap().len()); });
    timeit("between_f64", || { std::hint::black_box(s.between(&Scalar::Float64(0.2), &Scalar::Float64(0.8), "both").unwrap().len()); });
    timeit("pct_change", || { std::hint::black_box(s.pct_change(1).unwrap().len()); });
    timeit("clip_f64", || { std::hint::black_box(s.clip(Some(0.2), Some(0.8)).unwrap().len()); });
    timeit("cumprod", || { std::hint::black_box(s.cumprod().unwrap().len()); });
    timeit("add_self", || { std::hint::black_box(s.add(&s).unwrap().len()); });
    timeit("gt_series_self", || { std::hint::black_box(s.gt(&s).unwrap().len()); });
}
