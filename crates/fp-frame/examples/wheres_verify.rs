...
fn mkf(off:i64,n:usize,seed:u64)->Series{
    ...
    let v:Vec<Scalar>=(0..n).map(|i| if sm(i,seed)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Float64((sm(i,seed)%100) as f64)}).collect();
...
fn dump(tag:&str, s:&Series){
    ...
    for (l,v) in idx.iter().zip(s.column().values().iter()){
        ...
        let val=match v{Scalar::Float64(f)=>if f.is_nan(){"NaN".into()}else{format!("{f:.4}")},Scalar::Null(_)=>"NaN".into(),o=>format!("{o:?}")};
        print!("{key}:{val} ");
    }
    println!();
}
