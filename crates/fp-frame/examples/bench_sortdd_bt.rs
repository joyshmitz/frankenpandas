...
fn main(){
    ...
    let v=Series::new("v",idx.clone(),Column::from_values((0..n).map(|i| Scalar::Float64((sm(i,7)%500000) as f64)).collect()).unwrap()).unwrap();
    t("sort_values", || { std::hint::black_box(v.sort_values(true).unwrap()); });
    t("drop_duplicates", || { std::hint::black_box(v.drop_duplicates().unwrap()); });
    t("nlargest50", || { std::hint::black_box(v.nlargest(50).unwrap()); });
}
