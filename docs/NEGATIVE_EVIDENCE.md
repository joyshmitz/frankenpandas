...
| Series.sort_values Float64 spread — cache-friendly stable pair sort (1q4q4) | 2M f64, sort_values(ascending), high-cardinality/spread (range ≫ n) | 47.89 ms | 65.0 ms | **0.74× (was 0.40×) — IMPROVED, residual structural** | ⚠️ IMPROVED on spread data, still a pandas loss. `argsort_with` uses the 8-pass LSD `radix_argsort_f64`, memory-bound on spread f64 (2M unique was 0.40×, 121ms). For ascending all-valid no-NaN/no-(-0.0) Float64 with **range ≫ row count** (implies few duplicates), a cache-friendly inline `(value,index)` STABLE sort replaces it (121→65ms, **1.86× FP-side**). BIT-IDENTICAL — both are stable sorts (ties in original-index order), the gate only routes for perf; the value+index sequence matches pandas `sort_values(kind='stable')` exactly (50k hash 17287329826983589384). Heavily-duplicated data (range ≤ n/4) STAYS on the radix (a comparison sort has no equal-run shortcut), so no tie-case regression. Residual vs pandas is structural (numpy's tuned C introsort). Guards: 50 fp-frame sort tests, 33 fp-conformance sort, fmt+clippy clean. Note: a `first`-style unstable pdqsort (rank's lever) is NOT usable here — sort_values' golden tie order is index-stable. **Int64 sibling (same range>n/4 guard, `i64::cmp`, no NaN/-0.0 edge): 2M unique i64 154→92.5ms (1.66× FP-side, 0.40×→0.66× vs pandas 61ms); card=1000 stays on the radix (no regression). Bit-identical (stable i64 sort == stable radix permutation).** |
...
| Series.rank Float64 — value-only pdqsort/stable argsort (1q4q4) | 2M f64, rank(average); unique vs tie-heavy (card=1000) | unique 39.9 / ties 121 ms | unique 19.0 / ties 37.9 ms | **unique 2.10× (was 0.38× LOSS); ties 3.19× (was 1.77×)** | ✅ FIXED — `rank_f64_slice_radix` used `fp_columnar::radix_argsort_f64` (8-pass LSD), whose memory-bound scatter loses to numpy's introsort on spread data (2M unique f64 was **0.38×, 105ms**). Replaced with a cache-friendly inline `(value, index)` pair sort: **value-only** `sort_unstable` for average/min/max/dense (within-tie order irrelevant; value-only key lets pdqsort exploit equal runs), **stable** `sort_by` for `first` (needs the original-index tie order). Tie-walk reads `pairs[k].0` (no `data[perm[k]]` indirection). Bit-identical. unique 105→19.0ms (5.5× FP-side, 0.38×→2.10×); ties 68→37.9ms (1.77×→3.19×). Oracle-EXACT all 5 methods + desc + pct. −0.0 routes to the comparator fallback (gated). Guards: 48 fp-frame rank tests, 17 fp-conformance rank, fmt+clippy clean. |
