...
fn main() {
    ...
    timeit("gb.diff(1) Int64 all-valid", || {
        std::hint::black_box(sa.groupby(&k).unwrap().diff(1).unwrap());
    });
    timeit("gb.diff(1) Int64 nullable ", || {
        std::hint::black_box(sn.groupby(&k).unwrap().diff(1).unwrap());
    });
}
