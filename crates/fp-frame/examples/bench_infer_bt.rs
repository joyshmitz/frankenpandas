...
fn dh(s:&Series,d:&mut u64){ for v in s.column().values().iter(){ let (tag,pay)=match v {
    Scalar::Float64(x)=>(1u64,x.to_bits()),
    Scalar::Null(NullKind::NaN)=>(2,0), Scalar::Null(NullKind::NaT)=>(3,0), Scalar::Null(NullKind::Null)=>(4,0),
    Scalar::Int64(x)=>(5,*x as u64), _=>(9,0) };
    *d=d.wrapping_mul(1099511628211).wrapping_add(tag); *d=d.wrapping_mul(1099511628211).wrapping_add(pay); }
    for l in s.index().labels().iter(){ if let fp_index::IndexLabel::Int64(x)=l { *d=d.wrapping_mul(1099511628211).wrapping_add(*x as u64);} } }
fn main(){
    let n=2_000_000usize; let idx=Index::from_range(0,n as i64,1);
    // canonical NaN-missing (leading Null so from_inferred path)
    let vn=Series::new("v",idx.clone(),Column::from_values((0..n).map(|i| if sm(i,1)%5==0 {Scalar::Null(NullKind::NaN)} else {Scalar::Float64((sm(i,9)%1000000) as f64)}).collect()).unwrap()).unwrap();
    let positions: Vec<i64> = (0..n).map(|i| (sm(i,3)%n as u64) as i64).collect();
    t("take_nan", || { std::hint::black_box(vn.take(&positions).unwrap()); });
    t("sort_values_nan", || { std::hint::black_box(vn.sort_values_na(true,"last").unwrap()); });
    t("drop_duplicates_nan", || { std::hint::black_box(vn.drop_duplicates().unwrap()); });
    ...
    for seed in 0..80u64 {
        ...
        let c=Column::from_values(vv); let c=match c {Ok(c)=>c,Err(_)=>continue};
        ...
        if let Ok(dd)=s.drop_duplicates(){ dh(&dd,&mut d); }
    }
