...
fn build(n:usize, card:usize) -> DataFrame {
    ...
    let iv:Vec<Scalar>=(0..n).map(|i| if sm(i,7)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((sm(i,9)%1000) as i64)}).collect();
    // Utf8 value: 20% missing, ~200 distinct
    let sv:Vec<Scalar>=(0..n).map(|i| if sm(i,11)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Utf8(format!("v{}", sm(i,13)%200))}).collect();
    ...
    DataFrame::new_with_column_order(Index::from_range(0,n as i64,1), map, vec!["k".into(),"iv".into(),"sv".into()]).unwrap()
}
...
fn main(){
    ...
    for card in [1000usize] {
        ...
        timeit(&format!("gb.ffill  Int64 n={n} card={card}"), || { std::hint::black_box(si.groupby(&ki).unwrap().ffill(None).unwrap()); });
        timeit(&format!("gb.ffill  Utf8  n={n} card={card}"), || { std::hint::black_box(ss.groupby(&ki).unwrap().ffill(None).unwrap()); });
        timeit(&format!("gb.fillna Int64 n={n} card={card}"), || { std::hint::black_box(si.groupby(&ki).unwrap().fillna(&Scalar::Int64(0)).unwrap()); });
        timeit(&format!("gb.fillna Utf8  n={n} card={card}"), || { std::hint::black_box(ss.groupby(&ki).unwrap().fillna(&Scalar::Utf8("X".into())).unwrap()); });
    }
}
