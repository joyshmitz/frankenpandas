//! Bench for `Column::sort_values` on a nullable Int64/Float64 column — the new
//! typed_radix_perm + scalar-gather path. Source is a LAZY nullable column
//! (`from_*_values_with_validity`); without the fast path, sort_values falls to
//! the O(n log n) na-last `compare_scalars_na_last` Scalar comparator.
//!
//! Same-binary A/B: TREATMENT = `col.sort_values(asc)` (radix perm + gather);
//! CONTROL = the exact pre-change fallthrough (index/&Scalar stable sort by
//! `compare_scalars_na_last`, then clone gather). A folded checksum of both
//! results is asserted equal so the win is honest.
//!
//! Run: cargo run -p fp-columnar --release --example bench_sort_null_gather -- 5000000 12
...
// Replica of the pre-change generic fallthrough (na-last Scalar comparator).
fn control_sort(col: &Column, ascending: bool) -> Vec<Scalar> {
    let vals = col.values();
    let mut idx: Vec<usize> = (0..vals.len()).collect();
    idx.sort_by(|&a, &b| na_last(&vals[a], &vals[b], ascending));
    ...
    for (label, col) in [("i64", &icol), ("f64", &fcol)] {
        for ascending in [true, false] {
            ...
            for _ in 0..iters {
                ...
                let out = col.sort_values(ascending).expect("sort");
            ...
            for _ in 0..iters {
                let t = std::time::Instant::now();
                let out = control_sort(col, ascending);
                best_c = best_c.min(t.elapsed().as_nanos());
                ck_c = ck_c.wrapping_add(fold(&out));
            }
            assert_eq!(ck_t, ck_c, "{label} asc={ascending}: treatment != control");
            println!(
                "sort_null {label} asc={ascending} len={len} iters={iters} \
