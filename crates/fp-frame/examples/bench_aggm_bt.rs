...
fn digest(df:&fp_frame::DataFrame)->u64{ let mut s=0u64; for c in ["sum","mean","std","var","min","max","prod"]{ if let Some(col)=df.column(c){ for v in col.values().iter(){ if let Scalar::Float64(f)=v { s=s.wrapping_mul(1099511628211).wrapping_add(f.to_bits()); } } } } s }
fn main(){
    ...
    let ki=Series::new("ki",idx.clone(),Column::from_values((0..n).map(|i| Scalar::Int64((sm(i,1)%500) as i64)).collect()).unwrap()).unwrap();
    t("agg_multi5_utf8", || { std::hint::black_box(v.groupby(&k).unwrap().agg(&["sum","mean","std","min","max"]).unwrap()); });
    t("agg_multi5_i64", || { std::hint::black_box(v.groupby(&ki).unwrap().agg(&["sum","mean","std","min","max"]).unwrap()); });
    let r=v.groupby(&k).unwrap().agg(&["sum","mean","std","var","min","max","prod"]).unwrap();
