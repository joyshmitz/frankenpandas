//! DataFrameGroupBy.agg_list(["count","first"]) on a Utf8 value @1M.
...
fn contig(n: usize, f: impl Fn(usize) -> String) -> Column {
    ...
    o.push(0usize);
    for i in 0..n {
        b.extend_from_slice(f(i).as_bytes());
        o.push(b.len());
    }
...
fn main() {
    ...
    let op = a.get(3).map(String::as_str).unwrap_or("count");
    ...
    for _ in 0..6 {
        let t = Instant::now();
        black_box(df.groupby(&["k"]).unwrap().agg_list(&[op]).unwrap());
        let e = t.elapsed().as_nanos();
        if e < best {
            best = e;
        }
    }
    println!("gb_agglist_{op} n={n} gc={gc}: best={best}ns");
}
