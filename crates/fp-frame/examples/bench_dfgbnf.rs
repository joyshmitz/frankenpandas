...
fn build(n:usize, utf8key:bool, card:usize)->DataFrame{
    let keys:Vec<Scalar>= if utf8key { (0..n).map(|i| Scalar::Utf8(format!("cat{}", sm(i,1)%card as u64))).collect() }
        else { (0..n).map(|i| Scalar::Int64((sm(i,1)%card as u64) as i64)).collect() };
    let a:Vec<Scalar>=(0..n).map(|i| if sm(i,7)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Float64((sm(i,9)%1000) as f64)}).collect();
    let b:Vec<Scalar>=(0..n).map(|i| if sm(i,3)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Float64((sm(i,5)%1000) as f64)}).collect();
...
fn main(){
    ...
    for (tag,utf8,card) in [("bounded-i64",false,1000usize),("eager-Utf8",true,1000usize)] {
        let df=build(n,utf8,card);
        timeit(&format!("dfgb.sum   {tag} nf64 x2"), || { std::hint::black_box(df.groupby(&["k".into()]).unwrap().sum().unwrap()); });
        timeit(&format!("dfgb.mean  {tag} nf64 x2"), || { std::hint::black_box(df.groupby(&["k".into()]).unwrap().mean().unwrap()); });
    }
}
