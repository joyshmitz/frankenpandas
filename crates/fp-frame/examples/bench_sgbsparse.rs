...
fn main() {
    ...
    timeit("sgb.sum  sparse-i64-key nullable-i64", || {
        std::hint::black_box(sn.groupby(&k).unwrap().sum().unwrap());
    });
    timeit("sgb.mean sparse-i64-key nullable-i64", || {
        std::hint::black_box(sn.groupby(&k).unwrap().mean().unwrap());
    });
    timeit("sgb.max  sparse-i64-key nullable-i64", || {
