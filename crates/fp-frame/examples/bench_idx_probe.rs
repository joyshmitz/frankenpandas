...
fn main(){
    ...
    timeit("idxmax", || { std::hint::black_box(s.idxmax().unwrap()); });
    timeit("idxmin", || { std::hint::black_box(s.idxmin().unwrap()); });
    // correctness: max at a known position
    let sc=Series::new("c", Index::new(vec![IndexLabel::Utf8("a".into()),IndexLabel::Utf8("b".into()),IndexLabel::Utf8("c".into()),IndexLabel::Utf8("d".into()),IndexLabel::Utf8("e".into())]),
        Column::from_values(vec![Scalar::Float64(3.0),Scalar::Null(NullKind::NaN),Scalar::Float64(9.0),Scalar::Float64(1.0),Scalar::Float64(9.0)]).unwrap()).unwrap();
    println!("idxmax(ties)={:?} idxmin={:?}", sc.idxmax().unwrap(), sc.idxmin().unwrap());
    ...
    for seed in 0..400u64 {
        ...
        let vv:Vec<Scalar>=(0..sz).map(|i|{
            let drop = match nan_rate {0=>false,1=>sm(i,seed*7+1)%4==0,2=>sm(i,seed*7+1)%2==0,_=>true};
            if drop {Scalar::Null(NullKind::NaN)} else {
        ...
        for (i,s) in vv.iter().enumerate(){
            if let Scalar::Float64(v)=s { if !v.is_nan(){
                if omax.is_none()||*v>bmax {bmax=*v;omax=Some(i);}
                if omin.is_none()||*v<bmin {bmin=*v;omin=Some(i);}
            }}
        }
        let s=Series::new("t", Index::from_range(0,sz as i64,1), Column::from_values(vv).unwrap()).unwrap();
        let gmax=s.idxmax().ok().map(|l| if let IndexLabel::Int64(x)=l {x as usize} else {usize::MAX});
        let gmin=s.idxmin().ok().map(|l| if let IndexLabel::Int64(x)=l {x as usize} else {usize::MAX});
        if gmax!=omax || gmin!=omin { fails+=1; if fails<=5 {println!("DIFF seed={seed} sz={sz} nan={nan_rate} neg={neg} max fp={gmax:?} oracle={omax:?} min fp={gmin:?} oracle={omin:?}");}}
    }
